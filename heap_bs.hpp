...
namespace DS
{
    class HeapBitset
    {
        ...
        public:
            ...
            //Creates a bitset with bit length x
            HeapBitset(uint32 len);
            //Creates a bitset with memory copied from the bs pointer
            HeapBitset(HeapBitset* bs);
            //Creates an empty bitset
            HeapBitset();

            HeapBitset();
            //Reallocates to bit length x(kinda useless ngl)
            bool Reallocate(uint32 len);
            //Copies all of the contents from the pointed bitset to this
            ...
            //no
            void SetPattern(uint32 start, uint32 times, uint32 patternLength, uint64 pattern);
            ...
            //I mean honestly those funcs are kinda meh so idk why youd wanna use it but im keeping it here cuz I spent time making it lol
